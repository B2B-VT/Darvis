...
function FaqItem({ item, dark }) {
  ...
  return (
    <div style={{ ...glassCard(dark), borderRadius: RADIUS.md, overflow: "hidden", marginBottom: 8 }}>
      <button
        onClick={() => setOpen(o => !o)}
      ...
      >
        <span style={{ fontFamily: SANS, fontSize: 14, fontWeight: 600, color: t.text, lineHeight: 1.4 }}>{item.q}</span>
      ...
      {open && (
        <div style={{ padding: "0 20px 16px", fontFamily: SANS, fontSize: 14, color: t.textSub, lineHeight: 1.7 }}>{item.a}</div>
      )}
    </div>
  );
}
...
export default function LandingPage({ onEnter, onNavigate, darkMode, onCourseClick, onProfClick }) {
  ...
  return (
    <div style={{ position: "relative", isolation: "isolate", fontFamily: SANS, color: t.text }}>
      ...
      {/* ── FAQ ──────────────────────────────────────────────────────────────── */}
      <section style={{ padding: isMobile ? "64px 22px" : "96px 64px", borderTop: `1px solid ${t.lineSoft}` }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <Reveal style={{ textAlign: "center", marginBottom: 40 }}>
            ...
            <h2 style={{ fontFamily: SERIF, fontWeight: 400, margin: 0, fontSize: "clamp(28px, 3vw, 40px)", letterSpacing: "-0.4px", color: t.text, lineHeight: 1.1 }}>
